...
- 検証済みで台帳に入っているのは城崎温泉と黒川温泉の入湯手形の2件だけ。**JRパスの券種価格・区間運賃・別府の市営温泉はまだ `_unverified`**
- コマンドは `npm run check:gate`（台帳の状態）・`npm run check:gate -- <file>`（記事の下見）・`npm run test:gate`（20件）
