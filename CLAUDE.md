...
- ⚠️ **期間表記を要求するのは「その fact にしか出てこない数値」だけ。** `¥50,000` は JRパス7日間（現行）と JR東日本パス10日間の両方の価格なので、これに期間を強制すると「JR East Pass は ¥50,000」という正しい文が落ちる。共有された値は素通りするので、そこはプロンプト側の指示で補う（機械で完全には塞げない）
