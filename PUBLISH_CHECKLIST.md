...
## japan-onsen-towns-worth-the-detour（自然ピラー2本目・2026-08-18 公開）

**この記事の事実はすべて各温泉地の公式ページ（自治体・観光協会・旅館組合）から取った。**推計・平均は一切していない＝紅葉記事と違い自前の計算は無い。出典は記事の §9 に全件リンクしてある。

- 料金の時点: 別府市営 **令和8年4月1日現在**（PDF料金表）／城崎 **2023-04-01 改定**（外湯800円・ゆめぱ1,500円）／黒川 **2023-10-01 改定**（手形1,500円）／草津 大滝乃湯1,200円・白旗の湯無料／渋 大湯800円・10:00〜17:00／野沢 寸志（賽銭箱）
- **英語圏の記事がほぼ全て間違えている点を1つ含む**: 城崎の外湯は7湯ではなく**6湯**（さとの湯が2024-03-31から長期休業・豊岡市公式）。これが記事の§4-2の核なので、**再開したら記事を直す**
- **渋温泉の九湯は宿泊者限定**（公式サイトの明記）。日帰りは大湯のみ。ここも英語記事が取り違えている

**更新が要る条件（この記事が古くなる順）**

- [ ] 城崎さとの湯が再開したら §3 の表・§4-2・ピン文面の「6湯」を直す
- [ ] 各町が料金改定を出したら表と §9 を直す（改定告知ページを出典にしているので、改定日ごと差し替える）
- [ ] `pinterest-kit/pin_data_en.py` の数字は記事の表と必ず一致させる（ピンは2〜3年生きる）

**リンク**

- 12Go: 福岡→別府の確定ルートを設置（`sub_id=japan-onsen-towns-worth-the-detour`）
- Klook は素のURLで設置（`c28-tokyo/1002-day-trips/`）＝LinkSwitcher が自動変換する
- [ ] GetYourGuide: 温泉地の日帰りツアーは**未設置**。発行済み30本に該当ルートが無いため、次の発行分で草津・別府枠を取る

**書き方の確認（済）**

- [x] §2.3 整合性: 現地を歩いた体を装っていない。「公式ページを読んで比べた」という立て付けで統一
- [x] 検証できなかったものを2件、検証できないと明記して落とした（タトゥー可否・泉質の効能表現）＝§7
- [x] 運営側の理屈（料率・報酬）を本文に書いていない
- [x] `[VERIFY]` 0件
- [x] 内部リンク6本（JRパス・IC・高速バス・Klookツアー・紅葉・パッキング）
